...
class GPU_interface(object):
    ...
    def compile_gpu_kernels(self):
        ...
        cu_file = os.path.join(sw_dir, 'cuda_anuga.cu')
    ...
    def compute_fluxes_ext_central_kernel(self, domain_object, timestep,  transfer_from_cpu=True, transfer_gpu_results=True):
        """
        compute flux

        Ensure transient data has been copied to the GPU via cpu_to_gpu_flux
        
        NOTE: domain_object parameter is required to match the signature of 
        compute_fluxes_ext_central in OpenMP version. We use self.domain instead.
        """
        import cupy as cp
    ...
    def extrapolate_second_order_edge_sw_kernel(self, domain_object, transfer_from_cpu=True, transfer_gpu_results=True, verbose=False):
        """
        ...
        NOTE: domain_object parameter is required to match the signature of 
        extrapolate_second_order_edge_sw in OpenMP version. We use self.domain instead.
        """
        import cupy as cp
    ...
    def update_conserved_quantities_kernel(self,
        ...
        NOTE: domain_object parameter is required to match the signature of 
        update_conserved_quantities in OpenMP version. We use self.domain instead.
    ...
    def protect_against_infinitesimal_and_negative_heights_kernel(self, domain_object, transfer_from_cpu=True, transfer_gpu_results=True, verbose=False):
        ...
        NOTE: domain_object parameter is required to match the signature of 
        protect_new in OpenMP version. We use self.domain instead.
        """
