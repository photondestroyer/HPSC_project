...
class RainfallSimulationConfig:
    ...
    def __init__(self, start_date_str=None, end_date_str=None):
        # Input files
        self.dem_file = "ROI2_smoothed_topography.tif" # DEM in UTM Zone 43N
        self.corrected_tiff = "ROI2_smoothed_topography_EPSG32643.tif" # Corrected DEM with explicit EPSG
        self.rainfall_csv = "IMERG_ROI2_2020.csv" # IMERG data
        self.bounding_box_csv = "bounder_new.csv" # Domain boundary (new coordinates)
        
        # Coordinate system
        self.target_epsg = 32643 # UTM Zone 43N (corrected from 32644)
        ...
        # Output
        self.output_dir = 'infiltration_runoff_June_ROI2_2020'
...
config = RainfallSimulationConfig(start_date_str="26-05-2020", end_date_str="30-06-2020")
...
# ============================================================================
# HELPER FUNCTIONS FOR TIFF METADATA
# ============================================================================

def get_tiff_details(tiff_path):
    """
    Extract coordinate system details from a GeoTIFF file.
    
    Returns:
        zone_num (int): UTM zone number
        proj4_string (str): PROJ4 string
        epsg_code (int): EPSG code
        hemisphere (str): 'north' or 'south'
    """
    from osgeo import gdal, osr
    
    print(f"\nReading GeoTIFF metadata: {tiff_path}")
    ...
    # Get EPSG code
    epsg_code = None
    if spatial_ref.GetAuthorityName(None) == 'EPSG':
        epsg_code = int(spatial_ref.GetAuthorityCode(None))
    
    # Get UTM zone
    zone_num = spatial_ref.GetUTMZone()
    if zone_num == 0:
        # Try to extract from EPSG code
        if epsg_code and 32600 <= epsg_code <= 32660:
            zone_num = epsg_code - 32600
        elif epsg_code and 32700 <= epsg_code <= 32760:
            zone_num = epsg_code - 32700
        else:
            raise ValueError("Could not determine UTM zone from TIFF")
    ...
    if epsg_code:
        if 32600 <= epsg_code <= 32660:
            ...
            # Check if projection string contains hemisphere info
        ...
        # Fallback: check centroid latitude
        geotransform = ds.GetGeoTransform()
        x_size = ds.RasterXSize
        y_size = ds.RasterYSize
        
        # Get center coordinates
        center_x = geotransform[0] + (x_size / 2) * geotransform[1]
        center_y = geotransform[3] + (y_size / 2) * geotransform[5]
    ...
    print(f"  PROJ4: {proj4_string[:80]}..." if len(proj4_string) > 80 else f"  PROJ4: {proj4_string}")
    
    return zone_num, proj4_string, epsg_code, hemisphere


def create_tiff_with_epsg(input_tiff, output_tiff, target_epsg):
    """
    Create a copy of a TIFF with explicit EPSG code set.
    This ensures ANUGA can properly read the coordinate system.
    """
    from osgeo import gdal, osr
    
    print(f"\nCreating corrected TIFF with EPSG:{target_epsg}")
    ...
    if src_ds is None:
        raise FileNotFoundError(f"Could not open {input_tiff}")
    
    # Create output with same data but explicit projection
    driver = gdal.GetDriverByName('GTiff')
    dst_ds = driver.CreateCopy(output_tiff, src_ds, options=['COMPRESS=LZW'])
    
    # Set explicit EPSG projection
...
# ============================================================================
# STEP 1: GET TIFF CRS WITH HEMISPHERE
# ============================================================================
print("\n" + "="*70)
print("STEP 1: READING GEOTIFF PROJECTION")
print("="*70)
zone_num, tiff_proj4, epsg_code, hemisphere = get_tiff_details(config.dem_file)
print(f"✓ TIFF Zone: {zone_num}, EPSG: {epsg_code}, Hemisphere: {hemisphere}")

# ============================================================================
# STEP 2: CREATE CORRECTED TIFF WITH EXPLICIT EPSG
# ============================================================================
print("\n" + "="*70)
print("STEP 2: PREPARING GEOTIFF")
print("="*70)
if not os.path.exists(config.corrected_tiff):
    create_tiff_with_epsg(config.dem_file, config.corrected_tiff, config.target_epsg)
else:
...
# ============================================================================
# STEP 3: CREATING ANUGA DOMAIN
# ============================================================================
print("\n" + "="*70)
print("STEP 3: CREATING ANUGA DOMAIN")
print("="*70)
...
# ============================================================================
# STEP 4: SET ZONE AND HEMISPHERE (CRITICAL FOR DEM LOADING)
# ============================================================================
# Set coordinate reference using the TIFF metadata
domain.geo_reference.set_zone(zone_num)
domain.geo_reference.set_hemisphere(corrected_hemisphere)
print(f"✓ Set UTM Zone {zone_num}, Hemisphere: {corrected_hemisphere}")
...
# ============================================================================
# STEP 8: CONFIGURING GPU ACCELERATION
# ============================================================================
print("\n" + "="*70)
...
if config.use_gpu and domain.get_multiprocessor_mode() == 2:
    print(f"\n💡 TIP: Open a terminal and run 'nvidia-smi' during simulation")
    print(f"         to verify GPU is being used!")
print("="*70)
...
class GreenAmptInfiltrationOperator(anuga.Rate_operator):
    ...
    **FIX 3**: Uses hard-coded constant max infiltration rate (25.9 mm/hr)
    ...
    SOIL_PROPS = {
        ...
        'tikra': {'lambda': 0.378, 'h_b': 0.1101, 'K_s': 9e-6}  # Example custom soil
    }
    
    def __init__(self, domain, 
                 hydraulic_conductivity='tikra',
                 soil_type='tikra',
                 soil_porosity=0.45,
                 initial_soil_moisture=0.35,
                 soil_bulk_density=1590.0,
                 rock_density=2650.0,
                 volume_fraction_coarse_fragments=0.0,
                 surface_water_minimum_depth=1.0e-7,
                 infiltration_threshold=0.001,  # minimum depth for infiltration (m)
                 description=None, label=None, logging=False, verbose=False):
        """
        ...
        self._max_infiltration_rate_mm_hr = 35 # mm/hr (for display)
        self._max_infiltration_rate = 35 * (1e-3 * 3600) # m/s (for calculation)
...
domain,
hydraulic_conductivity='tikra', # Can use 'sand', 'loam', 'clay', etc., or custom value in m/s
soil_type='tikra',
soil_porosity=0.45,
